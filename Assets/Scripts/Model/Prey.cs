...
public class Prey : Animal
{
    ...
    public void ShowState(float deltaTime)
    {
        if (this.ID == 0 && this.AnimalType == AnimalType.Prey)
            Debug.Log("—ќ—“ќяЌ»≈: " + AnimalType + " - " + ID + " - " + CurrentAdaptiveState + "\n");
    }
    ...
    public override void GiveBirth()
    {
        ...
        {
            Prey child = AnimalManager.SpawnPrey(CurrentTile, this, this.Genome); // myTODO: здесь метод передачи генома от родител€ ребенку Genome.Inheritance
