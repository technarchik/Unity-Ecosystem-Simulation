...
public class MigrationEvent : Event
{
    ...
    public override void OnEventStart(World world, float severity, int durationInDays)
    {
        ...
        if (UnityEngine.Random.Range(0, 2) == 0)
        {
            ...
            while (amount > 0)
            {
                Predator newPrey = world.AnimalManager.SpawnPredator(spawnTile, null, Genome.CreateGenome(AnimalType.Predator));
        ...
        else
        {
            ...
            while (amount > 0)
            {
                Prey newPrey = world.AnimalManager.SpawnPrey(spawnTile, null, Genome.CreateGenome(AnimalType.Prey));
                int ageRand = UnityEngine.Random.Range(0, 4);
                if (ageRand == 0)
                {
                    newPrey.setElder();
